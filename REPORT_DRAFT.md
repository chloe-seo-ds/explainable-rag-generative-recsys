...
2. [Introduction and Problem Statement](#1-introduction-and-problem-statement)
   - 1.1 [Disclaimer (AI Tool Usage)](#11-disclaimer-ai-tool-usage)
   - 1.2 [Target Audience](#12-target-audience)
   - 1.3 [Three Paradigms and Our Contribution](#13-three-paradigms-and-our-contribution)
3. [Data Description and Preprocessing](#2-data-description-and-preprocessing)
4. [Models and Methods](#3-models-and-methods)
5. [Implementation Details — Configuration and Running Code](#4-implementation-details--configuration-and-running-code)
6. [Experiments and Results](#5-experiments-and-results)
7. [Discussion](#6-discussion)
8. [Future Work](#7-future-work)
9. [Limitations and Responsible Use](#8-limitations-and-responsible-use)
10. [Video Presentation](#9-video-presentation)
...
### 1.1 Disclaimer (AI Tool Usage)

Portions of this work were developed with assistance from Claude Code (Anthropic's CLI agent, claude-opus-4-7). Specifically, Claude Code was used for: iterating on notebook scaffolding (data loading, evaluation utilities), debugging Colab/Drive integration issues, drafting commit messages, and producing the first draft of this report. The substantive design decisions — paradigm selection, anchor papers, evaluation framework, methodological choices around log-likelihood scoring vs title generation, and interpretation of results — are our own. All code was reviewed and tested before inclusion. Where Claude Code suggested optimizations (e.g., the prompt-KV-cache scoring function in `next_steps/`), we ran functional smoke tests to verify correctness against a naive reference implementation. References to specific papers are the authors' selections and were sourced from the recommender-systems literature, not generated by the AI.

### 1.2 Target Audience

This report is written for two audiences. The **primary audience** is practitioners building text-rich recommender systems — ML engineers and applied scientists at companies with catalogs of books, articles, products, or other text-bearing items who need to choose between three increasingly popular paradigms and want a controlled, side-by-side comparison rather than three isolated papers. The **secondary audience** is researchers in recommendation, retrieval, and applied LLM evaluation who want a reproducible benchmark and a documented case study in how evaluation protocol choice (single-title generation vs log-likelihood scoring) can change conclusions about model capability. Both audiences should leave with: (a) a clear sense of which paradigm fits which constraint profile, (b) a reusable notebook framework for evaluating new methods on the same Amazon Books benchmark, and (c) an awareness of how easily methodological choices can over- or under-state a paradigm's true performance.

### 1.3 Three Paradigms and Our Contribution
...
## 4. Implementation Details — Configuration and Running Code
...
We identified several directions for follow-up work and scaffolded code for two of them in the `Notebooks/next_steps/` folder so a future runner can pick them up directly.

### 7.1 Full-Catalog Log-Likelihood Evaluation (Scaffolded)

The shared-pool log-likelihood evaluation (Section 5.8) addresses the structural unfairness on a 100-item pool, but we do not have a full-catalog (6,117-item) log-likelihood number for the generative paradigm. The full-catalog comparison currently mixes evaluation protocols: BPR-MF scores all items by dot product, RAG retrieves top-50 then reranks, and generative uses title generation. A full-catalog log-likelihood number would let us close that asymmetry.

We scaffolded two notebooks for this in `Notebooks/next_steps/`:

- **`01_full_catalog_kvcache.ipynb`** — exact, brute-force full-catalog log-likelihood with prompt-KV-cache reuse. Estimated cost: ~3 days on A100, ~6 days on L4. We verified the scoring function against a naive reference on a tiny GPT-2 model locally and achieved bit-exact agreement (after adding explicit `position_ids` — see notebook for implementation notes).

- **`02_full_catalog_two_stage.ipynb`** — first-token prefilter + full log-likelihood on the top-N survivors. Approximate but ~10× faster (~30–75 hours total). Includes a stage-1 ground-truth-recall diagnostic so the user can tune `TOP_N_STAGE1` based on observed coverage.

Both notebooks ship with `SMOKE_TEST = True` defaults so the user can verify the pipeline on 3 users before committing to a multi-day run.

### 7.2 Other Directions

- **Larger generative backbone.** All experiments use Qwen2.5-3B. Repeating the QLoRA fine-tuning on a 7B–13B model would test whether the gap between log-likelihood-evaluated generative (26.67%) and RAG (28.29%) closes or inverts.
- **Hallucination filtering.** The 32–37% hallucination rate is the biggest deployment blocker. A grounded-decoding constraint (restrict the LLM to mention only titles in the user's history) or a post-hoc claim verifier would be a natural extension.
- **User study on explanation utility.** All explanation metrics are automatic; we have no evidence that users find these explanations helpful or that they affect trust/click-through. A small user study on the 100 explanation samples would close that gap.
- **Cross-domain validation.** Replicate the full pipeline on MovieLens or Amazon Beauty to test whether the paradigm rankings transfer to domains with different text characteristics (short metadata vs long reviews vs taxonomic categories).
- **Online latency benchmarking.** All latency numbers are offline mean/p95 on heterogeneous hardware. A controlled benchmark on identical hardware with realistic query load would make the latency comparison apples-to-apples.
...
A recorded walkthrough of this project (7–15 minutes) covering motivation, methods, results, and a live demo of the unified evaluation notebook is available at:

**YouTube URL:** [PLACEHOLDER — paste link here before submission]

The video covers the same content as this report at a higher level, with screen-shared walkthroughs of:
1. `04_unified_evaluation.ipynb` — cross-paradigm comparison tables and charts
2. The key methodological finding (title generation vs log-likelihood scoring, Section 5.8)
3. The next-steps scaffolding in `Notebooks/next_steps/`
...
| 6 | `03_2_genrec_likelihood_eval.ipynb` | Paradigm 3: Log-likelihood scoring (shared pool) |
| 7 | `04_unified_evaluation.ipynb` | Cross-paradigm comparison tables and charts |
| (future) | `next_steps/01_full_catalog_kvcache.ipynb` | Future work — exact full-catalog log-likelihood |
| (future) | `next_steps/02_full_catalog_two_stage.ipynb` | Future work — approximate full-catalog two-stage |

### C. Scale Caps for LLM Inference
...
| NB03.2 | `EVAL_ONLY` | True (N=2,820) | Stopped early to keep the L4 run under one day |
